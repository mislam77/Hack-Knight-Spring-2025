...
export const DataCharts = () => {
  ...
  const [authUser, setAuthUser] = useState<{ uid: string | null }>({
    uid: null,
  });
          ...
          const formattedTransactions = userTransactions.map((transaction) => {
            ...
            try {
              if (transaction.createdAt) {
                ...
                processedTransaction.createdAt = new Date();
                console.warn(
                  "Transaction missing createdAt date, using current date:",
                  transaction
                );
              }
            } catch (error) {
              console.error(
                "Error processing transaction timestamp:",
                error,
                transaction
              );
        ...
        ? [
            ...
            ...(transactions.some((t) => !t.category)
              ? transactions
                  ...
                  .reduce<Array<{ name: string; value: number }>>(
                    (acc, curr) => {
                      const existing = acc.find(
  ...
  const handleImageLoad = () => {
    setIsImageLoaded(true); // Update state to indicate that the image has loaded
  };

  // Show loading state
  if (loading) {
    return (
      <div className="max-w-full max-h-full mx-auto bg-white rounded-lg mt-3">
        <div className="w-full h-full flex flex-col items-center relative">
          ...
          />
          {isImageLoaded && (
            <p className="absolute top-9 text-center mt-4">Please wait...</p>
          )}
        </div>
      </div>
    );
  ...
  if (!authUser.uid) {
    return (
      <div className="py-10 text-center">
        Please sign in to view your transaction data
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 w-full max-w-7xl mx-auto h-full my-15">
      ...
      <div className="flex flex-col space-y-6 h-full">
        ...
        <div className="flex-1 flex items-stretch mt-10">
          <div className="w-full h-full">
